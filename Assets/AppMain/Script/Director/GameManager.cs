...
public class GameManager : MonoBehaviour
{
    ...
    // 次フェーズ遷移待ち状態を維持する時間
    private const float PhaseWaitTime = 1.0f;
    ...
    void Start()
    {
        ...
        _guiManager = GUIManager.instance;  //GameObject.Find("GUIManager").GetComponent<GUIManager>(); // GUIManager.instance;
    ...
    void Update()
    {
        ...
        // 敵ターンはスムーズにまわすために
        // 毎回見る
        if (_turnManager.ActiveTurnCharacter.GetCharacterData.IsEnemy && _enemyTurnFlg)
        {
            //Debug.Log("敵ターン中");
        ...
        else
        {
            ...
            switch (_currentPhase)
            {
                ...
                case TurnPhase.PHASE_GAME_START:
                    ...
                    _cameraManager.ChangeActiveVirtualCamera(_turnManager.ActiveTurnCharacter);
                    ...
                    // 自分のターンが終了していないなら、継続
                    if (_turnManager.ActiveTurnStatus == TurnManager.TurnStatus.ACTIVE_PLAYER_TURN)
                    {
                        // フェーズ遷移待ちに変更
    ...
    private void WaitForCorutine()
    {
        ...
        {
            _currentCoroutine = StartCoroutine(ChangePhase(PhaseWaitTime, _nextPhase));
        }
    }
    ...
    private void SelectBlock()
    {
        ...
        if (mapBlock != null)
        {
            ...
            // 選択したブロックを強調
            mapBlock.EmphasizeBlock(SelectBlockMode.SELECT_BLOCK);
    ...
    public void ShowMoveArea()
    {
        ...
        if (character != null)
        {
            ...
            {
                // 選択したブロックを強調
                moveAreaBlock.EmphasizeBlock(SelectBlockMode.SELECT_MOVE);
            }
    ...
    private void MoveCharacter(MapBlock mapBlock)
    {
        ...
        if (_moveAllowArea.Contains(mapBlock))
        {
            ...
            _mapManager.SetUsedMapBlock(_selectingCharacter.PositionX, _selectingCharacter.PositionZ, false);
    ...
    public void ShowAttackArea()
    {
        ...
        Debug.Log("ShowAttackArea" + "X座標：" + character.PositionX + " Z座標：" + character.PositionZ);
        ...
        if (character != null)
        {
            ...
            _atkAllowArea = _mapManager.SearchAttackAreaByCharacter(_selectingCharacter.PositionX, _selectingCharacter.PositionZ);

            foreach (MapBlock atkAreaBlock in _atkAllowArea)
            {
                // 選択したブロックを強調
                atkAreaBlock.EmphasizeBlock(SelectBlockMode.SELECT_ATTACK);
            }

            // フェーズ切り替え
            _currentCoroutine = StartCoroutine(ChangePhase(PhaseWaitTime, TurnPhase.PHASE_ATK_TURN));
    ...
    private void AttackCharacter(MapBlock mapBlock)
    {
        ...
        if (_atkAllowArea.Contains(mapBlock))
        {
            ...
            {
                Debug.Log("攻撃対象キャラクター名：" + targetCharacter.GetCharacterData.CharacterName);
    ...
    public void Attack(Character target, Character selectingCharacter)
    {
        ...
        // 攻撃相手の方向を向く
        selectingCharacter.TurnToTarget(target.PositionX, target.PositionZ);
    ...
    private void NextAction()
    {
        ...
        {
            // フェーズ切り替え(行動選択状態)
            _currentCoroutine = StartCoroutine(ChangePhase(PhaseWaitTime, TurnPhase.PHASE_MY_TURN));
        ...
        else
        {
            ...
            _currentCoroutine = StartCoroutine(ChangePhase(PhaseWaitTime, TurnPhase.PHASE_NEXT_TURN));
    ...
    {
        // 次の行動開始キャラが敵の場合
        if (_turnManager.ActiveTurnCharacter.GetCharacterData.IsEnemy)
        {
            // 敵行動開始フェーズ設定
            _currentCoroutine = StartCoroutine(ChangePhase(PhaseWaitTime, TurnPhase.PHASE_ENEMY_TURN));
            SetPhaseWait(TurnPhase.PHASE_ENEMY_TURN);
            _enemyTurnFlg = true;
